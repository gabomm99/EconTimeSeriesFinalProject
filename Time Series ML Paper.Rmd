...
```{r}
library(glmnet)
train_vde = final_merged_vde[1:1877,]
test_vde = final_merged_vde[1878:3754,]
...
rsq <- 1 - sse/sst
mse <- (sum((y_vde - y_predicted)^2))/1877
mse
rsq
AICc
opt_lambda
...
```{r}
train_vht = final_merged_vht[1:1877,]
test_vht = final_merged_vht[1878:3754,]
...
tLL <- - deviance(opt_LASSO_model_vht)
k <- opt_LASSO_model_vht$df
n <- opt_LASSO_model_vht$nobs
AICc_vht <- -tLL+2*k
...
mse_vht <- (sum((y_vht - y_predicted_vht)^2))/1877
mse_vht
rsq_vht
AICc_vht
opt_lambda_vht
